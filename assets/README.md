...
`cursor-wand.svg` — the site-wide default cursor everywhere except the hero's own canvas cursors below. A `wandCursor()` DOM overlay (see `index.html`) puts a small pink-outline dot exactly on the real mouse position; this static wand body (held at a constant -30deg tilt, tip pinned per its own `transform-origin`) and 3 small twinkles each just get a new `transform` target on every mousemove, pointed at a spot offset 18/24px down-right of the dot (`WAND_GAP`) so the wand settles a little apart from it rather than sitting on top of whatever the dot itself is hovering (a link's text, say). The trailing/easing itself is a plain CSS `transition` on `transform` — a different duration per element (220ms wand, 300/380/460ms for the 3 sparkles) — so the compositor runs it, not a per-frame JS loop; the elements also get `will-change:transform` so animating them doesn't repaint the ~270 masked paths in this file on every move, just recomposite a cached layer. (An earlier version drove all this from a `requestAnimationFrame` loop that ran forever, 60×/sec, even at rest — measurably slower; this version costs nothing once the mouse stops.) The 3 twinkle shapes are traced from `FINAL FOLDER/cursore/Sparkle Wand.svg`; each of the 3 sparkles starts its 200ms shape-cycle on a different one of the 3 shapes, so they read as scattered variation rather than one synced blink. Cropped from `FINAL FOLDER/cursore/Cursor Wand.svg` (81×176) down to just the wand body (81×118) — the source file's own 3 trailing sparkle-icon paths were dropped since the overlay draws its own; a later re-export (`cursore/Vector.svg`) turned out identical in complexity (same 136 paths, same colors, ~0.18px export-origin shift) so it wasn't worth swapping in. Kept as SVG rather than rasterized (the project's usual move for a heavily-repeated pixel-art export, see `wizzy.png` below) because it's one static image, not redrawn per-instance. Native `cursor:none` is set site-wide behind `@media (pointer:fine)` to make room for it; `.cell--graph` / `--dot` / `--check` / `--iso` keep their own explicit `cursor: url()` rules below, which still win the cascade, and `wandCursor()` also hides its own DOM overlay there so it doesn't sit on top of them. The work carousel's `.qc` cards get the same full hide — their own hover tint + corner cut-outs are the feedback, the wand's just extra motion on top — and the footer's `.ft__frame` game drops to just the plain dot (`is-simple`, opacity-only on the wand body + sparkles) rather than going fully invisible, since gameplay still needs *some* cursor but the full wand+sparkles was distracting mid-game. All of this is delegated `mouseover`/`mouseout` on `document` with `closest()`, not per-element `mouseenter`/`mouseleave` — `.qc` cards are built by the work carousel after `wandCursor()` runs, so binding directly to them at setup time would've silently missed every card. `cursor.svg`, the old pink-dot pointer this replaced as the hero's default, is unused now — kept for reference.
