...
`cursor.svg` — plain pink dot, the hero's default cursor (`cursor:url(assets/cursor.svg) 13 13, auto` on `.hero`). No glow/shadow — a bare 6.5px-radius circle on a 26×26 canvas, hotspot dead centre.

`cursor-wand.svg` (currently unused, kept for reference) — a from-scratch site-wide cursor replacement: a pink-outline dot pinned to the real mouse position, a wand body (fixed -30deg tilt) and 3 sparkles trailing it on staggered CSS transitions, hidden over the hero's canvas cursors and the work carousel's `.qc` cards, dropped to just the dot inside the footer's `.ft__frame` game. Shelved back to the plain `cursor.svg` above for QA — it was adding real overhead (a `requestAnimationFrame` loop running forever even at rest, a `closest()` walk on every mousemove) and, even after fixing that, was proving distracting/slow in exactly the places game feel and card-hover feedback matter most. Cropped from `FINAL FOLDER/cursore/Cursor Wand.svg`; sparkle shapes from `FINAL FOLDER/cursore/Sparkle Wand.svg`. Revisit with a fuller spec before re-enabling.
