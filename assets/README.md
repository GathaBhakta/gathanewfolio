...
`cursor-wand.svg` — the site-wide default cursor everywhere except the hero's own canvas cursors below. A `wandCursor()` DOM overlay (see `index.html`) translates + rotates this static wand body to face the mouse's travel direction, tip pinned exactly under the pointer, plus a small twinkle beside the tip that cycles through 3 shapes (traced from `FINAL FOLDER/cursore/Sparkle Wand.svg`) every 200ms. Cropped from `FINAL FOLDER/cursore/Cursor Wand.svg` (81×176) down to just the wand body (81×118) — the source file's own 3 trailing sparkle-icon paths were dropped since the overlay draws its own. Kept as SVG rather than rasterized (the project's usual move for a heavily-repeated pixel-art export, see `wizzy.png` below) because it's one static image, not redrawn per-instance. Native `cursor:none` is set site-wide behind `@media (pointer:fine)` to make room for it; `.cell--graph` / `--dot` / `--check` / `--iso` keep their own explicit `cursor: url()` rules below, which still win the cascade, and `wandCursor()` also hides its own DOM overlay there so it doesn't sit on top of them. `cursor.svg`, the old pink-dot pointer this replaced as the hero's default, is unused now — kept for reference.
